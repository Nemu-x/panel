...
def test_host_with_wireguard_amazia(access_token):
    core = create_core(access_token)
    inbound_list = get_inbounds(access_token)
    assert inbound_list, "No inbounds available for host updates"
    inbound = inbound_list[0]
    create_response = client.post(
        ...
        headers={"Authorization": f"Bearer {access_token}"},
        json={
            "remark": unique_name("test_host_wireguard_amazia"),
            "address": ["127.0.0.1"],
            "port": 443,
            "sni": ["test_sni.com"],
            "inbound_tag": inbound,
    ...
    assert create_response.status_code == status.HTTP_201_CREATED
    host_id = create_response.json()["id"]
    assert create_response.json()["wireguard_amazia"]["jc"] == 4
    assert create_response.json()["wireguard_amazia"]["jmin"] == 40
    assert create_response.json()["wireguard_amazia"]["h1"] == "1"
    assert create_response.json()["wireguard_amazia"]["i1"] == "some_mimic_data"
    assert create_response.json()["wireguard_amazia"]["s2"] is None
    ...
    update_response = client.put(
        ...
        headers={"Authorization": f"Bearer {access_token}"},
        json={
            "remark": unique_name("test_host_wireguard_amazia_updated"),
            "address": ["127.0.0.2"],
            "port": 443,
            "sni": ["test_sni_updated.com"],
            "inbound_tag": inbound,
    ...
    assert update_response.status_code == status.HTTP_200_OK
    assert update_response.json()["wireguard_amazia"]["jc"] == 8
    assert update_response.json()["wireguard_amazia"]["jmin"] == 50
    assert update_response.json()["wireguard_amazia"]["h1"] == "2"
    assert update_response.json()["wireguard_amazia"]["i1"] is None
